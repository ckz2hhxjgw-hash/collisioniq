...
export default async function handler(req, res) {
  ...
  // Use captured map image (base64 JPEG) or fallback to link button
  const lastPt = preCollisionTrack[preCollisionTrack.length - 1];
  const gmapsUrl = lastPt
    ? `https://www.google.com/maps/search/?api=1&query=${lastPt.lat},${lastPt.lng}`
    : 'https://www.google.com/maps/search/?api=1&query=36.154,-115.168';
  const mapSectionHtml = mapImageData
    ? `<a href="${gmapsUrl}" style="display:block;text-decoration:none;"><img src="${mapImageData}" width="560" style="width:100%;border-radius:8px;display:block;border:0;" alt="Pre-Collision Track Map"></a>`
    : `<a href="${gmapsUrl}" style="display:inline-block;background:#f1f5f9;border:1.5px solid #E5EEF2;border-radius:8px;padding:8px 14px;font-size:12px;font-weight:700;color:#4C84FF;text-decoration:none;">&#x1F5FA;&#xFE0F; View Impact Location on Map &rarr;</a>`;
